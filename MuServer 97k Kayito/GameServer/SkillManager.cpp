...
int CSkillManager::GetSkillNumber(int index, int level)
{
	...
	switch (index)
	{
		...
		}// Custom Orbs -> Weapon Skills (19 to 23 from Skill.txt)
		case GET_ITEM(12, 20):
		{
			return SKILL_FALLING_SLASH; // Falling Slash (or SKILL_FALLING_SLASH)
		}
		case GET_ITEM(12, 21):
		{
			return SKILL_LUNGE; // Lunge (or SKILL_LUNGE)
		}
		case GET_ITEM(12, 22):
		{
			return SKILL_UPPERCUT; // Uppercut (or SKILL_UPPERCUT)
		}

		case GET_ITEM(12, 23):
		{
			return SKILL_CYCLONE; // Cyclone (or SKILL_CYCLONE)
		}

		case GET_ITEM(12, 24):
		{
			return SKILL_SLASH; // Slash (or SKILL_SLASH)
		}
	}
...
{
	// If it's not a weapon skill, allow it immediately
	if (index != SKILL_DEFENSE &&
		index != SKILL_FALLING_SLASH &&
		index != SKILL_LUNGE &&
		index != SKILL_UPPERCUT &&
		index != SKILL_CYCLONE &&
		index != SKILL_SLASH &&
		index != SKILL_TRIPLE_SHOT &&
		index != SKILL_FIRE_BREATH &&
		index != SKILL_POWER_SLASH)
	{
		return true;
	}

	// Check if the player has a weapon with this skill
...
int CSkillManager::AddSkillWeapon(LPOBJ lpObj, int index, int level)
{
	...
	{
		lpObj->WeaponSkillAdded[index] = true;
...
int CSkillManager::DelSkillWeapon(LPOBJ lpObj, int index, int level)
{
	...
	if (count == 1 && lpObj->WeaponSkillAdded[index] != false)
	{
		if ((slot = this->DelSkill(lpObj, index)) >= 0)
		{
			lpObj->WeaponSkillAdded[index] = false;
