...
{
	if (lpObj->Level >= this->GetCharacterMaxLevel(lpObj))
	{
...
int CObjectManager::GetCharacterMaxLevel(LPOBJ lpObj)
{
	if (lpObj->Reset >= 31)
	{
		return 1000;
...
bool CObjectManager::CharacterUsePotion(LPOBJ lpObj, CItem* lpItem)
{
	...
	switch (lpItem->m_Index)
	{
		case GET_ITEM(14, 0):
		{
			HPValue = 25 + (int)(((lpObj->MaxLife + lpObj->AddLife) * gServerInfo.m_ApplePotionRate) / 100);
		...
		case GET_ITEM(14, 1):
		{
			HPValue = 75 + (int)(((lpObj->MaxLife + lpObj->AddLife) * gServerInfo.m_SmallLifePotionRate) / 100);
		...
		case GET_ITEM(14, 2):
		{
			HPValue = 150 + (int)(((lpObj->MaxLife + lpObj->AddLife) * gServerInfo.m_MidleLifePotionRate) / 100);
		...
		case GET_ITEM(14, 3):
		{
			HPValue = 350 + (int)(((lpObj->MaxLife + lpObj->AddLife) * gServerInfo.m_LargeLifePotionRate) / 100);
		...
		case GET_ITEM(14, 4):
		{
			MPValue = 100 + (int)(((lpObj->MaxMana + lpObj->AddMana) * gServerInfo.m_SmallManaPotionRate) / 100);
		...
		case GET_ITEM(14, 5):
		{
			MPValue = 250 + (int)(((lpObj->MaxMana + lpObj->AddMana) * gServerInfo.m_MidleManaPotionRate) / 100);
		...
		case GET_ITEM(14, 6):
		{
			MPValue = 600 + (int)(((lpObj->MaxMana + lpObj->AddMana) * gServerInfo.m_LargeManaPotionRate) / 100);
