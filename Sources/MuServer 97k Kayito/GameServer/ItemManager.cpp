...
{
	//gNotice.GCNoticeSend(aIndex, 1, "ITEM USE PACKET RECEIVED");
	...
	if (lpItem->m_Index >= GET_ITEM(12, 20) && lpItem->m_Index <= GET_ITEM(12, 24))
	{
		printf("[ORB] PACKET RECEIVED\n");
		return;
	}

	//if (lpItem->m_Index >= GET_ITEM(12, 20) && lpItem->m_Index <= GET_ITEM(12, 24))
	//{
	//	int skill = gSkillManager.GetSkillNumber(lpItem->m_Index, lpItem->m_Level);

	//	if (skill >= 0)
	//	{
	//		int slot = gSkillManager.AddSkill(lpObj, skill, lpItem->m_Level);
	...
	// UPDATED: Upper bound changed from 12, 19 to 12, 24
